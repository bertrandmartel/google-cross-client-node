...
(function() {
    ...
    app.Record = Backbone.Model.extend({
        ...
        url: function() {
            return baseUrl + '/admin/devices/' + (this.isNew() ? '' : this.id + '/');
        }
    });

    app.RecordCollection = Backbone.Collection.extend({
        model: app.Record,
        url: baseUrl + '/admin/devices/',
    ...
    app.HeaderView = Backbone.View.extend({
        ...
        addNew: function() {
            if (this.$el.find('[name="email"]').val() === '') {
                alert('Please enter a email.');
            } else if (this.$el.find('[name="hash"]').val() === '') {
                ...
                this.model.save({
                    ...
                    success: function(model, response) {
                        if (response.success) {
                            app.headerView.model.set({ email: '', hash: '' });
